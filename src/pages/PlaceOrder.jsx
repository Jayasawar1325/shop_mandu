...
import { ShopContext } from '../context/ShopContext'
 ...
 const PlaceOrder = () => {
  ...
  return (   
    <div className='flex flex-col sm:flex-row justify-between gap-4 pt-5 sm:pt-14 min-h-[890vh] border-t '>
...
<Title text1={'DELIVERY'} text2={'INFORMATION'}/>
  ...
  <input placeholder='Phone' type="number" className='border border-gray-300 rounded py-1.5 px-3.5 w-full'  />
...
<div onClick={()=>setMethod('stripe')}  className="flex items-center gap0-3 border p-2 px-3 cursor-pointer">
  <p className={`min-w-3.5 h-3.5 border rounded-full  ${method==='stripe'? 'bg-green-400':''}`}></p>
  <img src={assets.stripe_logo} className='h-5 mx-4' alt="" />
</div>
<div onClick={()=>setMethod('razorpay')} className="flex items-center gap0-3 border p-2 px-3 cursor-pointer">
...
<div onClick={()=>setMethod('cod')} className="flex items-center gap-3 border p-2 px-3 cursor-pointer">
  ...
  <div className='w-full text-end mt-8'>
    <button onClick={()=>navigate('/orders')} className='bg-black text-white text-sm px-6 py-4'>Place Orders</button>
  </div>
</div>
</div>
    </div>
